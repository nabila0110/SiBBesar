...
<head>
    <meta charset="utf-8">
    <title>Laporan Buku Besar - {{ $company->name }}</title>
    <style>
        @page {
...
<body>
    <div class="header">
        @if($company->logo && file_exists(public_path('images/companies/' . $company->logo)))
            <img src="{{ public_path('images/companies/' . $company->logo) }}" class="logo" alt="Logo">
        @else
    ...
    @if($dari_tanggal || $sampai_tanggal)
        <div class="report-date">
            Periode: 
            @if($dari_tanggal){{ date('d/m/Y', strtotime($dari_tanggal)) }}@else-@endif
            s/d 
            @if($sampai_tanggal){{ date('d/m/Y', strtotime($sampai_tanggal)) }}@else-@endif
        </div>
    @endif
